...
BOOL CreateWindowGL (GL_Window* window)									// This Code Creates Our OpenGL Window
{
	...
	{
		//scrnResX=GetSystemMetrics(SM_CXSCREEN);
		//scrnResY=GetSystemMetrics(SM_CYSCREEN);
		if(ChangeScreenResolution (window->init.width, window->init.height, window->init.bitsPerPixel) == FALSE)//if (ChangeScreenResolution (scrnResX, scrnResY, window->init.bitsPerPixel) == FALSE)//
		{
...
int WINAPI WinMain (HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	{
		MessageBox(0,e.what(),"",0);
	}catch(...)
	{
		MessageBox(0,(LPCSTR)const_cast<unsigned char*>(glGetString(glGetError())),"",0);
	}

...
{
	if(row * _numVertsPerRow + col>_heightmap.size())return -1;
	return _heightmap[row * _numVertsPerRow + col];
}

void Terrain::setHeightmapEntry(int row, int col, int value)
{
	int index = row * _numVertsPerRow + col;
	if(index>_heightmap.size()){
		MessageBox(0,"","",0);return;
	}
	_heightmap[index] = value;
	vertices[index]._y=value;
}

bool Terrain::computeVertices()
{
	HRESULT hr = 0;
	vertices=new TerrainVertex[_numVertices];
...
bool Terrain::computeIndices()
{
	...
	//	WORD* indices = 0;
	//indices.reserve(_numTriangles * 3 );
	indices=new WORD[_numTriangles*3];
...
void Terrain::getHeightMapCoord(float x, float z,float &col, float &row)
{
...
bool Terrain::draw(bool drawTris)
{
	...
	glDrawElements(GL_TRIANGLES,_numTriangles*3,GL_UNSIGNED_SHORT,indices);
//	glBegin(GL_TRIANGLES);
//	for(int i=0;i<_numTriangles * 3;++i)
//	{
//		TerrainVertex *v=&(vertices[indices[i]]);
